...
class GenericFieldUpdates(BaseModel):
    ...
    effectiveDate: Optional[str] = None  # YYYY-MM-DD
    ...
    # General deal terms (used by many docs)
...
class ChatRequest(BaseModel):
    ...
    current_fields: dict
    document_type: Optional[str] = None  # None = selection/home page mode


class ChatResponse(BaseModel):
    reply: str
    fields: GenericFieldUpdates
    suggested_document_type: Optional[str] = None  # returned in selection mode
...
async def chat(request: ChatRequest):
    ...
    if doc_type is None:
        # Selection mode: help user identify the right document
        ...
        try:
            ...
            extract_task = acompletion(
                ...
                response_format=SelectionExtract,
            ...
            return ChatResponse(
                reply=reply,
                fields=GenericFieldUpdates(),
                suggested_document_type=selection.suggested_document_type,
            )
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))

    else:
        # Document creation mode: specific document type
        ...
        try:
            ...
            extract_task = acompletion(
                model=MODEL,
                messages=extract_messages,
                response_format=GenericFieldUpdates,
                extra_body=EXTRA_BODY,
            )
            reply_response, extract_response = await asyncio.gather(reply_task, extract_task)
