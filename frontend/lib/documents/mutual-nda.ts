...
function buildStandardTerms(data: Record<string, string>): string {
...
function buildMutualNdaPreviewMarkdown(data: Record<string, string>): string {
  return buildCoverPage(data) + '\n\n---\n\n' + buildStandardTerms(data)
}
...
export const mutualNdaConfig: DocumentConfig = {
  slug: 'mutual-nda',
  name: 'Mutual NDA',
  description: 'A standard mutual non-disclosure agreement for sharing confidential information between two parties.',
  filename: 'Mutual-NDA.md',
  defaultData,
  buildPreviewMarkdown: buildMutualNdaPreviewMarkdown,
}
